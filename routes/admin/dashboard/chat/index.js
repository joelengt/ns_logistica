...
// Obteniendo contenido del chat room - chat con un amigo - Limite 0 to 20
			...
			Users.findById({'_id': user_friend_id}, function (err, user_friend) {
				...
				for(var f = 0; f <= Chatroom.messages.length - 1; f++) {
					...
					if(message.user_id === user_id) {
						...
						message.name  = req.user.full_name
...
// Obteniendo contenido del chat room - chat con un amigo - More
app.post('/room/:room_id/:user_friend_id/more', isLoggedIn, function (req, res) {
	if(req.user.permiso === users_type.onwers || 
	   req.user.permiso === users_type.admins ||
	   req.user.permiso === users_type.officers ||
	   req.user.permiso === users_type.viewer) {
